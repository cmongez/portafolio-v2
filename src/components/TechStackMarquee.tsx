...
const technologies = [
  ...
  { name: "SQL Server", icon: getIconUrl("microsoftsqlserver") },
...
function TechItem({ tech }: { tech: { name: string; icon: string } }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 min-w-[80px] group/item transition-transform hover:scale-110 duration-300">
      ...
      <span className="text-xs font-medium text-neutral-400 group-hover/item:text-neutral-200 transition-colors">
        {tech.name}
      </span>
    </div>
  );
